...
3. Run `Install.bat` on Windows, or use Python `3.14.x` and install dependencies manually with `pip install -r requirements.txt`.
...
3. Execute `Install.bat` no Windows, ou use Python `3.14.x` e instale as dependencias manualmente com `pip install -r requirements.txt`.
...
3. Ejecuta `Install.bat` en Windows, o usa Python `3.14.x` e instala las dependencias manualmente con `pip install -r requirements.txt`.
4. Comando manual: `python src/main/photo_dedup.py`.
